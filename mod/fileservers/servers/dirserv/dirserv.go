...
func (m *Manager) ServerWebFileRequest(w http.ResponseWriter, r *http.Request) {
	...
	requestPath := wdfs.ToSlash(filepath.Clean(r.RequestURI))
	...
	if len(pathChunks) == 0 {
		...
		for _, fsh := range fshs {
			html += getItemHTML(fsh.Name, wdfs.ToSlash(filepath.Join(r.RequestURI, fsh.UUID)), true, "-", "-")
		}
		html += getPageFooter()
		w.Write([]byte(html))
		...
		if fshAbs.IsDir(rpath) {
			...
			for _, entry := range entries {
				...
				html += getItemHTML(entry.Name(), wdfs.ToSlash(filepath.Join(r.RequestURI, entry.Name())), entry.IsDir(), finfo.ModTime().Format("2006-01-02 15:04:05"), byteCountIEC(finfo.Size()))
			}
