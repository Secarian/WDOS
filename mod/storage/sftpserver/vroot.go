...
// Wrapper for the wdfs File to provide missing functions
type wrappedWDFile struct {
	file wdfs.File
}

func newWDFileWrapper(wdfile wdfs.File) *wrappedWDFile {
	return &wrappedWDFile{file: wdfile}
}

func (f *wrappedWDFile) Name() string {
	return f.file.Name()
}

func (f *wrappedWDFile) Size() int64 {
...
func (f *wrappedWDFile) Mode() os.FileMode {
	stat, err := f.file.Stat()
	if err != nil {
		return 0
	}
...
func (f *wrappedWDFile) ModTime() time.Time {
	stat, err := f.file.Stat()
	if err != nil {
		return time.Time{}
	}
...
func (f *wrappedWDFile) IsDir() bool {
	stat, err := f.file.Stat()
	if err != nil {
		return false
	}
...
func (f *wrappedWDFile) Sys() interface{} {
	return nil
}

func (f *wrappedWDFile) ReadAt(b []byte, off int64) (int, error) {
	return f.file.ReadAt(b, off)
}

func (f *wrappedWDFile) WriteAt(b []byte, off int64) (int, error) {
...
func (fs *root) Filewrite(r *sftp.Request) (io.WriterAt, error) {
	if wdfs.ToSlash(filepath.Dir(r.Filepath)) == "/" {
