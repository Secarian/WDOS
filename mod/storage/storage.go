...
func NewStoragePool(fsHandlers []*fs.FileSystemHandler, owner string) (*StoragePool, error) {
	...
	return &StoragePool{
		Owner:           owner,
		OtherPermission: wdfs.FsReadOnly,
...
func (s *StoragePool) HasHigherOrEqualPermissionThan(a *StoragePool) bool {
	if s.OtherPermission == wdfs.FsReadOnly && a.OtherPermission == wdfs.FsReadWrite {
		return false
	} else if s.OtherPermission == wdfs.FsDenied && a.OtherPermission != wdfs.FsDenied {
		return false
	}
	return true
}
...
func (s *StoragePool) GetFsHandlerByUUID(uuid string) (*fs.FileSystemHandler, error) {
	...
	return nil, wdfs.ErrFSHNotFOund
}
