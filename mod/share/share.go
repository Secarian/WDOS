...
func (s *Manager) HandleOPGServing(w http.ResponseWriter, r *http.Request, shareID string) {
	...
	filename := wdfs.Base(shareEntry.FileRealPath)
		...
		if targetFshAbs.IsDir(fileRuntimeAbsPath) {
			...
			if directDownload {
				if relpath != "" {
					...
					if !strings.HasPrefix(wdfs.ToSlash(reqPath), wdfs.ToSlash(rootPath)) {
					...
					w.Header().Set("Content-Disposition", "attachment; filename*=UTF-8''"+strings.ReplaceAll(url.QueryEscape(wdfs.Base(targetFilepath)), "+", "%20"))
					...
					if targetFsh.RequireBuffer {
						...
						http.ServeContent(w, r, wdfs.Base(targetFilepath), fi.ModTime(), f)
					}
					...
					targetZipFilename := filepath.Join(tmpFolder, wdfs.Base(fileRuntimeAbsPath)) + ".zip"
					...
					if targetFsh.RequireBuffer {
						...
						localBuff = filepath.Join(tmpFolder, uuid.NewV4().String(), wdfs.Base(fileRuntimeAbsPath))
					...
					//Build a filelist
					err := filesystem.WDZipFile([]*filesystem.FileSystemHandler{zippingSourceFsh}, []string{zippingSource}, nil, targetZipFilename, false)
					if err != nil {
					...
					//Serve thje zip file
					w.Header().Set("Content-Disposition", "attachment; filename*=UTF-8''"+strings.ReplaceAll(url.QueryEscape(wdfs.Base(shareOption.FileRealPath)), "+", "%20")+".zip")
					w.Header().Set("Content-Type", r.Header.Get("Content-Type"))
				...
				s := t.ExecuteString(map[string]interface{}{
					...
					"filename":     wdfs.Base(fileRuntimeAbsPath),
			...
			if directDownload {
				...
				w.Header().Set("Content-Disposition", "attachment; filename=\""+wdfs.Base(shareOption.FileVirtualPath)+"\"")
				...
				if filesystem.FileExists(fileRuntimeAbsPath) {
					...
					if targetFsh.RequireBuffer {
						...
						http.ServeContent(w, r, wdfs.Base(fileRuntimeAbsPath), fi.ModTime(), f)
					}
				}
			} else if directServe {
				...
				s := t.ExecuteString(map[string]interface{}{
					...
					"size":        filesystem.GetFileDisplaySize(fsize, 2),
					"modtime":     timeString,
					"downloadurl": "/share/download/" + id + "/" + wdfs.Base(fileRuntimeAbsPath),
