...
func ValidateOption(options *FileSystemOption) error {
	...
	if !FileExists(options.Path) && !wdfs.IsNetworkDrive(options.Filesystem) {
		return errors.New("Path not exists, given: " + options.Path)
	}

	//Check if access mode is supported
	if !inSlice([]string{wdfs.FsReadOnly, wdfs.FsReadWrite}, options.Access) {
	...
	if !inSlice(wdfs.GetSupportedFileSystemTypes(), options.Filesystem) {
		return errors.New("Not supported file system type: " + options.Filesystem)
	}

	//Check if mount point exists
	if options.Automount && options.Mountpt == "" && !wdfs.IsNetworkDrive(options.Filesystem) {
