...
func (l FTPFSAbstraction) Create(filename string) (wdfs.File, error) {
...
func (l FTPFSAbstraction) Open(filename string) (wdfs.File, error) {
	return nil, wdfs.ErrOperationNotSupported
}
func (l FTPFSAbstraction) OpenFile(filename string, flag int, perm os.FileMode) (wdfs.File, error) {
	return nil, wdfs.ErrOperationNotSupported
}
...
func (l FTPFSAbstraction) VirtualPathToRealPath(subpath string, username string) (string, error) {
	return wdfs.GenericVirtualPathToRealPathTranslator(l.uuid, l.hierarchy, subpath, username)
}

func (l FTPFSAbstraction) RealPathToVirtualPath(fullpath string, username string) (string, error) {
	return wdfs.GenericRealPathToVirtualPathTranslator(l.uuid, l.hierarchy, fullpath, username)
}
...
func (l FTPFSAbstraction) Glob(realpathWildcard string) ([]string, error) {
	return []string{}, wdfs.ErrOperationNotSupported
}
...
func (l FTPFSAbstraction) ReadDir(filename string) ([]fs.DirEntry, error) {
	...
	for _, entry := range entries {
		entryFilename := wdfs.ToSlash(filepath.Join(filename, entry.Name))
...
func filterFilepath(rawpath string) string {
	rawpath = wdfs.ToSlash(filepath.Clean(strings.TrimSpace(rawpath)))
