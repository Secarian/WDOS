...
import (
	...
	"imuslab.com/wdos/mod/filesystem/wdfs"
...
func (l LocalFileSystemAbstraction) Create(filename string) (wdfs.File, error) {
...
func (l LocalFileSystemAbstraction) Open(filename string) (wdfs.File, error) {
	return os.Open(filename)
}
func (l LocalFileSystemAbstraction) OpenFile(filename string, flag int, perm os.FileMode) (wdfs.File, error) {
...
func (l LocalFileSystemAbstraction) VirtualPathToRealPath(subpath string, username string) (string, error) {
	rpath, err := wdfs.GenericVirtualPathToRealPathTranslator(l.UUID, l.Hierarchy, subpath, username)
	if err != nil {
		return "", err
	}
...
func (l LocalFileSystemAbstraction) RealPathToVirtualPath(fullpath string, username string) (string, error) {
	...
	fullpath = wdfs.ToSlash(fullpath)
	if strings.HasPrefix(fullpath, wdfs.ToSlash(filepath.Clean(l.Rootpath))) {
		fullpath = strings.TrimPrefix(fullpath, wdfs.ToSlash(filepath.Clean(l.Rootpath)))
	}

	vpath, err := wdfs.GenericRealPathToVirtualPathTranslator(l.UUID, l.Hierarchy, wdfs.ToSlash(fullpath), username)
	//fmt.Println("REAL TO VIRTUAL", wdfs.ToSlash(filepath.Clean(l.Rootpath)), fullpath, vpath)
	return vpath, err
}
