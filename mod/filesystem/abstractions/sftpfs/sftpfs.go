...
func (s SFTPFileSystemAbstraction) Chmod(filename string, mode os.FileMode) error {
	filename = wdfs.GenericPathFilter(filename)
...
func (s SFTPFileSystemAbstraction) Create(filename string) (wdfs.File, error) {
	filename = wdfs.GenericPathFilter(filename)
...
func (s SFTPFileSystemAbstraction) Open(filename string) (wdfs.File, error) {
	filename = wdfs.GenericPathFilter(filename)
...
func (s SFTPFileSystemAbstraction) OpenFile(filename string, flag int, perm os.FileMode) (wdfs.File, error) {
	filename = wdfs.GenericPathFilter(filename)
...
func (s SFTPFileSystemAbstraction) Rename(oldname, newname string) error {
	oldname = wdfs.GenericPathFilter(oldname)
	newname = wdfs.GenericPathFilter(newname)
...
func (s SFTPFileSystemAbstraction) VirtualPathToRealPath(subpath string, username string) (string, error) {
	rpath, err := wdfs.GenericVirtualPathToRealPathTranslator(s.uuid, s.hierarchy, subpath, username)
	if err != nil {
		return "", err
	}
	if !(len(rpath) >= len(s.mountFolder) && rpath[:len(s.mountFolder)] == s.mountFolder) {
		...
		rpath = wdfs.ToSlash(filepath.Join(s.mountFolder, rpath))
	}
	return rpath, nil
}
...
func (s SFTPFileSystemAbstraction) RealPathToVirtualPath(fullpath string, username string) (string, error) {
	...
	return wdfs.GenericRealPathToVirtualPathTranslator(s.uuid, s.hierarchy, fullpath, username)
}
...
func (s SFTPFileSystemAbstraction) Glob(realpathWildcard string) ([]string, error) {
	realpathWildcard = wdfs.GenericPathFilter(realpathWildcard)
...
func (s SFTPFileSystemAbstraction) Walk(root string, walkFn filepath.WalkFunc) error {
	root = wdfs.GenericPathFilter(root)
