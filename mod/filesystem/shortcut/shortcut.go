...
func ReadShortcut(shortcutContent []byte) (*wdfs.ShortcutData, error) {
...
func GenerateShortcutBytes(shortcutTarget string, shortcutType string, shortcutText string, shortcutIcon string) []byte {
	...
	if shortcutType == "module" && utils.FileExists(wdfs.ToSlash(filepath.Join("./app/", filepath.Dir(shortcutIcon), "/desktop_icon.png"))) {
		shortcutIcon = wdfs.ToSlash(filepath.Join(filepath.Dir(shortcutIcon), "/desktop_icon.png"))
	}

	//Clean the shortcut text
	shortcutText = wdfs.FilterIllegalCharInFilename(shortcutText, " ")
