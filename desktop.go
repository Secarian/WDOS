	...
	if !userFsa.FileExists(userDesktopPath) {
		...
		if fs.FileExists(templateFolder) {
			...
			for _, tfile := range templateFiles {
				...
				userFsa.WriteFile(wdfs.ToSlash(filepath.Join(userDesktopPath, filepath.Base(tfile))), input, 0755)
			}
		}
	}
		...
		*/
		...
		for _, file := range files {
			...
			if utils.StringInArray([]string{".png", ".jpg", ".gif"}, ext) {
				imageList = append(imageList, wdfs.ToSlash(filepath.Join(rpath, file.Name())))
			}
		}
